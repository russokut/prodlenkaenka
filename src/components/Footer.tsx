  ...
  return (
    <footer className="bg-white pt-20 pb-10 px-4 border-t border-slate-100">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        ...
        {/* Newsletter */}
        <div className="flex flex-col gap-6">
          <h4 className="text-lg font-display font-bold text-slate-800">Рассылка</h4>
          <p className="text-slate-500 text-sm">Подпишитесь на наши новости и акции</p>
          <form className="flex flex-col gap-3">
            <input 
              type="email" 
              placeholder="Ваш email"
              className="px-4 py-3 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-brand-blue outline-none text-sm"
            />
      ...
      <div className="max-w-7xl mx-auto mt-20 pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-slate-400">
        ...
        <div className="flex gap-6">
          <Link to="/privacy" className="hover:text-brand-blue">Политика конфиденциальности</Link>
          <Link to="/terms" className="hover:text-brand-blue">Условия использования</Link>
        </div>
      </div>
    </footer>
  );
