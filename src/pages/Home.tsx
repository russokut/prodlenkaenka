...
import { Star, Heart, Shield, Users, BookOpen, Music, ArrowRight, CheckCircle2 } from 'lucide-react';
    ...
    <div className="overflow-hidden">
      ...
      <section className="relative min-h-[90vh] flex items-center px-4 py-20">
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-brand-mint/10 text-brand-mint rounded-full font-bold text-sm mb-6">
              <Star className="w-4 h-4 fill-brand-mint" />
              <span>Лучший детский центр 2023</span>
            </div>
      ...
      <section className="py-24 px-4 bg-brand-cream/50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: 'Мини-сад', age: '1.5 - 7 лет', img: 'https://picsum.photos/seed/garden/400/500', href: '/mini-garden', color: 'border-brand-yellow' },
              { title: 'Продленка', age: '7 - 10 лет', img: 'https://picsum.photos/seed/school/400/500', href: '/after-school', color: 'border-brand-blue' },
              { title: 'Творчество', age: '3 - 10 лет', img: 'https://picsum.photos/seed/art/400/500', href: '/creativity', color: 'border-brand-coral' },
              { title: 'Летний клуб', age: '5 - 10 лет', img: 'https://picsum.photos/seed/camp/400/500', href: '/summer-camp', color: 'border-brand-mint' },
            ].map((dir, i) => (
              <Link 
                key={i}
                to={dir.href}
      ...
      <section className="py-24 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500">Примерный график занятий в нашем мини-саду</p>
          </div>

          <div className="relative">
            <div className="absolute left-1/2 top-0 bottom-0 w-1 bg-brand-cream -translate-x-1/2 hidden md:block" />
            
            <div className="flex flex-col gap-12">
              {[
                { time: '07:30 - 09:00', title: 'Встреча детей', desc: 'Прием детей, свободные игры, зарядка', icon: Users },
                { time: '09:00 - 09:30', title: 'Завтрак', desc: 'Полезный и вкусный завтрак', icon: Heart },
                ...
                { time: '12:30 - 13:00', title: 'Обед', desc: 'Полноценный горячий обед', icon: Heart },
              ].map((item, i) => (
                <div key={i} className={cn("flex flex-col md:flex-row items-center gap-8", i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse")}>
                  <div className="flex-1 text-center md:text-right">
                    {i % 2 === 0 && (
                      <>
                        <div className="text-brand-blue font-bold text-xl mb-2">{item.time}</div>
                        <h4 className="text-2xl font-bold text-slate-800 mb-2">{item.title}</h4>
                        <p className="text-slate-500">{item.desc}</p>
                      </>
                    )}
                  </div>
                  <div className="w-16 h-16 bg-white border-4 border-brand-cream rounded-full flex items-center justify-center z-10 shadow-lg shrink-0">
