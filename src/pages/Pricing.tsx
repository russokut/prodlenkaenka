...
export default function Pricing() {
  ...
  const pricingData = {
    'mini-garden': [
      { title: 'Полный день', price: '35 000', period: 'месяц', features: ['08:00 - 19:00', '5-разовое питание', 'Все развивающие занятия', 'Соляная пещера', 'Прогулки 2 раза'] },
      { title: 'Половина дня', price: '22 000', period: 'месяц', features: ['08:00 - 13:00', 'Завтрак и обед', 'Утренние занятия', 'Прогулка'], popular: true },
      { title: 'Разовое посещение', price: '2 500', period: 'день', features: ['Полный день', 'Питание включено', 'Все занятия'] },
    ],
    'after-school': [
      { title: 'Абонемент Месяц', price: '18 000', period: 'месяц', features: ['12:00 - 19:00', 'Обед и полдник', 'Помощь с уроками', 'Творчество', 'Прогулки'] },
      { title: 'Разовое посещение', price: '1 200', period: 'день', features: ['Полный день', 'Питание включено', 'Помощь с уроками'] },
    ],
    'extra': [
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-32">
          {pricingData[activeTab as keyof typeof pricingData].map((plan, i) => (
            <div key={i} className={cn("glass-card p-10 flex flex-col gap-8 relative", plan.popular && "border-4 border-brand-coral")}>
              {plan.popular && (
        ...
        <div className="bg-white rounded-[50px] p-12 md:p-20 shadow-xl flex flex-col lg:flex-row gap-12 items-center">
          ...
          <div className="lg:w-1/2 grid grid-cols-2 gap-6">
            ...
            <div className="p-8 bg-brand-coral/10 rounded-3xl">
              <h4 className="font-bold text-brand-coral mb-2">Скидка 15%</h4>
              <p className="text-sm text-slate-500">При оплате за полгода вперед</p>
            </div>
            <div className="p-8 bg-brand-blue/10 rounded-3xl">
              <h4 className="font-bold text-brand-blue mb-2">Скидка 20%</h4>
              <p className="text-sm text-slate-500">Для многодетных семей</p>
            </div>
            <div className="p-8 bg-brand-yellow/10 rounded-3xl">
              <h4 className="font-bold text-brand-yellow mb-2">Кешбэк</h4>
              <p className="text-sm text-slate-500">Возвращаем 5% на бонусный счет</p>
            </div>
          </div>
        </div>
      </div>
